...
export default function ContentDetailPage() {
  ...
  const contentSlugOrId = params.slug || params.id;
  const { toast } = useToast();
  const { user, isAuthenticated } = useAuth();
  const { requireAuth, AuthPrompt } = useAuthPrompt("interact with this content");
  const [selectedRating, setSelectedRating] = useState(0);

  // Fetch content by slug or ID - treat as ID only if it matches UUID format, otherwise slug
  const isUUID = contentSlugOrId?.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i);
  const { data: content, isLoading: isLoadingContent, error: contentError} = useQuery<Content>({
    queryKey: isUUID
      ? ["/api/content", contentSlugOrId]
      : ["/api/content/slug", contentSlugOrId],
    enabled: !!contentSlugOrId,
  });
  ...
  const likeMutation = useMutation({
    ...
    onSuccess: () => {
      if (content?.id) {
        queryClient.invalidateQueries({ queryKey: ["/api/content/slug", contentSlugOrId] });
        queryClient.invalidateQueries({ queryKey: ["/api/content", contentSlugOrId] });
      }
      toast({
        title: "Liked!",
  ...
  // Loading state
  if (isLoadingContent || isLoadingAuthor) {
    return (
